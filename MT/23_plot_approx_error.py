...
NODEMAP_NAME = 'Nodemap_F10000_alpha0.5_w200_h200_esize0.125.txt'
PLOT_PATH = os.path.join('williams_fitting', 'results', 'diagrams', 'approx_error')
...
fit_results = os.path.join('williams_fitting', 'results', 'MT_parameter_study_fit_method.csv')

df_res = pd.read_csv(fit_results)
df_res = df_res[df_res['Filename'] == NODEMAP_NAME]
df_res = df_res[df_res['r_min'] == R_MIN]
df_res = df_res[df_res['angle_gap'] == ANGLE_GAP]
df_res = df_res[df_res['tick_size'] == TICK_SIZE]

df = pd.DataFrame(columns=['x', 'FE', 'Fit', f'{SIGMA}_norm', 'N'])
...
for N in [1, 2, 3, 4, 5]:
    ...
    for n in range(1, N+1):
        A_n = df_res[f'a{n}'].mean()
        B_n = df_res[f'b{n}'].mean()
        a.append(A_n)
        b.append(B_n)
    ...
    df_int = pd.DataFrame()
    df_int = df_int.assign(Method=['Fit'] * len(ligament_x))
    ...
    df_pivot[f'{SIGMA}_norm'] = (df_pivot['Fit'] - df_pivot['FE']) / df_pivot['FE']
...
plt.savefig(os.path.join(PLOT_PATH, f'MT_{NODEMAP_NAME[:-4]}_{SIGMA}_norm_log.png'), dpi=600)
plt.close()
# plt.show()
